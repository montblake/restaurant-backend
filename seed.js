...
const clearDB = async () => {
  ...
  console.log('All menu types deleted from collection');
  await Course.deleteMany({});
  console.log('All courses deleted from collection');
  await MenuItem.deleteMany({})
  console.log('All menu items deleted from collection');
}

let dinnerId, drinksId, dessertId;
...
const generateCourses = (menuTypesFromDB) => {
  const dinnerId = menuTypesFromDB.find(menuType => menuType.name === "Dinner")._id;
  const drinksId = menuTypesFromDB.find(menuType => menuType.name === "Drinks")._id;
  const dessertId = menuTypesFromDB.find(menuType => menuType.name === "Dessert")._id;
...
const generateMenuItems = (coursesFromDB) => {
  const chilledId = coursesFromDB.find(course => course.name === "Chilled")._id;
  const firstId = coursesFromDB.find(course => course.name === "First")._id;
  const mainId = coursesFromDB.find(course => course.name === "Main")._id;
  const cocktailsId = coursesFromDB.find(course => course.name === "Cocktails")._id;
  const wineId = coursesFromDB.find(course => course.name === "Wine")._id;
  const beerId = coursesFromDB.find(course => course.name === "Beer")._id;
  const zeroProofId = coursesFromDB.find(course => course.name === "Zero Proof")._id;
  const dessertsId = coursesFromDB.find(course => course.name === "Desserts")._id;
  const coffeeId = coursesFromDB.find(course => course.name === "Coffee")._id;
  const digestifsId = coursesFromDB.find(course => course.name === "Digestifs")._id;
...
const createMenuTypes = async () => {
  // Insert new data into the DB
  const menuTypesFromDB = await MenuType.create(menuTypes);
  console.log(`Created ${menuTypesFromDB.length} menu types`);
  return menuTypesFromDB;
}

const seedDB = async () => {
  await clearDB();
  const menuTypesFromDB = await createMenuTypes();
  const coursesWithMenuType = generateCourses(menuTypesFromDB);
  const coursesFromDB = await Course.create(coursesWithMenuType);
  console.log(`Created ${coursesFromDB.length} courses`);
  const menuItemsWithCourse = generateMenuItems(coursesFromDB);
  const menuItemsFromDB = await MenuItem.create(menuItemsWithCourse);
